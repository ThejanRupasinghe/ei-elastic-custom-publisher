...
public class ElasticMediationFlowObserver implements MessageFlowObserver {
     ...
     *
     * @return Map object containing the configurations
    ...
    private String resolvePassword(String passwordInConfig) {
        ...
        if (secureVaultPassword) {
            ...
            SecretResolver secretResolver = SecretResolverFactory.create(serverConf.getDocumentElement(), true);
            ...
            if (ElasticObserverConstants.PASSWORD_ALIAS.equals(password)) {
                log.error("Wrong password alias in Secure Vault. Use alias: " + ElasticObserverConstants.PASSWORD_ALIAS);
                password = null;
            } else {
                if (log.isDebugEnabled()) {
